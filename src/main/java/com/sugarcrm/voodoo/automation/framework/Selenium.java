...
public class Selenium implements IAutomation {
	 ...
	 * @throws Exception
	 */
	public Selenium(Voodoo voodoo, Properties props, InterfaceType browserType)
			throws Exception {
		this.voodoo = voodoo;
		this.props = props;
	...
	public void go(String url) throws Exception {
		voodoo.log.info("Selenium: going to URL and switching to window: "
				+ url);
		browser.get(url);
	...
	@Override
	public String getAttribute(VControl control, String attribute)
			throws Exception {
		voodoo.log.info("Selenium: getting attribute: " + attribute
				+ " for control: " + control.toString());
	...
	public void waitOn(VControl control) throws Exception {
		...
		long explicitWait = Long.parseLong(Utils.getCascadingPropertyValue(
				props, "12000", "perf.explicit_wait"));
	...
	public String getSelected(VSelect select) throws Exception {
		voodoo.log.info("Selenium: getting selected value from control: "
				+ select.toString());
	...
	public void select(VSelect select, String value) throws Exception {
		voodoo.log.info("Selenium: selecting value from control: "
				+ select.toString());
		By by = Selenium.getBy(select.getHook().hookStrategy,
				select.getHook().hookString);
		Select dropDownList = new Select(browser.findElement(by));
	...
	private WebDriver getBrowser(InterfaceType browserType) throws Exception {
		...
		long implicitWait = Long.parseLong(props
				.getProperty("perf.implicit_wait"));
		if (System.getProperty("headless") == null) {
			java.awt.Dimension screenSize = Toolkit.getDefaultToolkit()
					.getScreenSize();
			webDriver
