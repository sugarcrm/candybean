...
public class Translations {
	...
	public static void Translate(String propPath) throws Exception {
		try {
			...
			database = getCascadingPropertyValue(translateProp, "null",
					"translate.database");
			module = getCascadingPropertyValue(translateProp, "null",
					"translate.module");
			language = getCascadingPropertyValue(translateProp, "null",
					"translate.language");
			testPath = getCascadingPropertyValue(translateProp, "null",
					"translate.testpath");
			outputDirPath = getCascadingPropertyValue(translateProp, "null",
	...
	/**
	 * first arg: database (translate_6_3, translate_6_2, translate_6_1) second
	 * arg: module (Account, Quotes, Contacts, etc) third arg: language (fr_FR,
	 * ja_JP, zh_CH, en_UK, etc) forth arg: testPath (path to the test that
	 * needs to be converted)
	 */
	public static void Translate(String db, String mod, String lang,
			String testP) throws Exception {
		try {
			// File translatePropertiesFilePath = new File(currentWorkingPath +
			// File.separator + "src" + File.separator + "main" + File.separator
			// + "resources" + File.separator + "translate.properties");
			// translateProp.load((new
			// FileInputStream(translatePropertiesFilePath)));
			database = db;
			module = mod;
			language = lang;
			testPath = testP;
			outputDirPath = currentWorkingPath + File.separator
					+ "translated_Files" + "_" + language;
	 ...
	 * @author wli
	 * 
	 *         A wrapper function that will run fileReaderWriter() once if the
	 *         given testpath is a single testfile or multiple times if it is a
	...
	private static void recurseTestFolder(String input) throws Exception {
		try {
			...
			if (inputFile.isFile()) { // no need to recurse -- perform
										// fileReaderWriter once
				fileReaderWriter(connect(con, database), module, language,
						input,
						outputDirPath + File.separator + inputFile.getName()
								+ "_" + language);
			} else { // else it is a path that could contain many files -
						// perform a check recursively
				File[] files = new File(input).listFiles();
				for (File file : files) {
					// for each file: if the file is of type java (fileformat)
					// and contains a module name from the ModuleList then
					// perform translation
					if (file.isFile()
							&& moduleExist(getFileModuleName(file.getName()))
							&& file.getName().contains(".java")) {
						// perform translation
						fileReaderWriter(connect(con, database),
								getFileModuleName(file.getName()), language,
								file.getAbsolutePath(), outputDirPath
										+ File.separator + file.getName() + "_"
										+ language);
					}
					if (file.isDirectory()) {
	 ...
	 * @param outputFile
	 * 
	 *            Scans the input file for a set of regex patters: assert, link
	 *            text, {variable}, module_tab, extramenu, and (data) and
	 *            outputs to another xml file with the appropriate tanslations
	 *            made. The converted file should be used with voodoo
	 * 
	 *            If a {variable} or (data) is found, no queries will be
	 *            performed module_tab and extramenu are ids that change
	 *            depending on the language that is chosen (this is a product
	 *            bug that should be filed, if filed, needs to be removed)
	 */
	private static void fileReaderWriter(Connection con, String module,
			String language, String inputFile, String outputFile)
			throws Exception {
		...
		try {
			...
			Pattern pattern_moduletab = Pattern.compile("id=\"moduleTab_("
					+ module + ")\"");
			...
			while (fileScanner.hasNextLine()) {
				...
				if (match_assert.find()) { // Assert replacement
					if (javaFile && match_variable.find()) { // java file format
																// check
																// (Voodoo2)
						System.out.println("Not finding translation for "
								+ match_assert.group(1));
						output.write(line + "\r\n"); // XML file format check
					} else if (!javaFile
							&& (match_variable.find() || match_pageNumber
									.find())) {
						System.out.println("Not finding translation for "
								+ match_assert.group(1));
						output.write(line + "\r\n");
						...
						if (javaFile) { // for java file format replacement
							String newLine = line
									.replace(
											assertSplit(match_assert.group(1)),
											databaseReplace(con, module,
													language,
													assertSplit(match_assert
															.group(1))));
							output.write(newLine + "\r\n");
						} else { // for XML file format
							String newLine = line.replace(
									match_assert.group(1),
									databaseReplace(con, module, language,
											match_assert.group(1)));
							output.write(newLine + "\r\n");
						}
					}
					...
					String newLine = line.replace(match_moduletab.group(1),
							databaseReplace(con, "SugarFeed", language, "All")
									+ "_" + module);
					output.write(newLine);
				} else if (match_menuextra_all.find()) {
					String newLine = line.replace(
							match_menuextra_all.group(1),
							databaseReplace(con, "SugarFeed", language,
									match_menuextra_all.group(1)));
					output.write(newLine);
				} else {
					output.write(line + "\r\n");
	...
	private static Connection connect(Connection con, String database)
			throws Exception {
		String serverName = getCascadingPropertyValue(translateProp,
				"10.8.31.10", "translate.serverName");
		String username = getCascadingPropertyValue(translateProp,
				"translator", "translate.username");
		String password = getCascadingPropertyValue(translateProp, "Sugar123!",
				"translate.password");
		System.out.println("connecting with: " + serverName + " " + username
				+ " " + password);
		try {
	 ...
	 * 
	 *         queries the module table that is passed. However, if the
	 *         translation does not exist within the current module, there will
	 *         be a search in all modules
	 */
	@SuppressWarnings("finally")
	private static String databaseReplace(Connection con, String module,
			String language, String english) throws Exception {
		String result = null;
		PreparedStatement pst = null;
		ResultSet rs = null;

		try {
			pst = con.prepareStatement("SELECT " + language + " FROM " + module
					+ " WHERE english = " + "\'" + english + "\'");
			rs = pst.executeQuery();

			// if there is a result due to the query, the translated value is
			// returned
			// else, there will be queries on all of the modules until a
			// translated value is found (it will return the first one it finds)
			if (rs.next()) {
				result = (rs.getString(1));
				System.out.println("Replaced english: " + english + " with "
						+ language + ": " + result);
			} else {
				System.out.println("Could not find the translation for "
						+ english + " in the " + module + " module");
				int counter = 0;
				...
				while (tables[counter] != null) {
					...
					pst_ifExists = con.prepareStatement("SHOW columns from `"
							+ tables[counter] + "` where field='" + language
							+ "'");
					rs_ifExists = pst_ifExists.executeQuery();

					// checks to see if the language is in that particular
					// module
					if (rs_ifExists.next()) {
						PreparedStatement pst_temp = null;
						ResultSet rs_temp = null;

						pst_temp = con.prepareStatement("SELECT " + language
								+ " FROM " + tables[counter]
								+ " WHERE english = " + "\'" + english + "\'");
						rs_temp = pst_temp.executeQuery();
						if (rs_temp.next()) {
							result = (rs_temp.getString(1));
							System.out.println("Replaced english: " + english
									+ " with " + language + ": " + result
									+ " from the " + tables[counter]
									+ " module");
							break;
						} else {
							System.out
									.println("Could not find the translation for "
											+ english
											+ " in the "
						...
						System.out
								.println("The table: " + tables[counter]
	...
	private static String assertSplit(String assertStr) throws Exception {
		...
		if (argumentListString.size() == 2)
			assertType = 0;
		else if (argumentListString.size() == 3)
			assertType = 1;
		else
			throw new Exception("Invalid Number of Arguments, got "
					+ argumentListString.size() + " argument(s)");
		...
		if (ret.substring(0, 1).equals("\"")
				&& ret.substring(ret.length() - 1, ret.length()).equals("\"")) {
	...
	private static String getCascadingPropertyValue(Properties props,
		...
		if (!props.containsKey(key) && value == "null") {
			System.out.println("Property file does not contain such key: "
					+ key);
			System.exit(0);
		}
	 ...
	 * @author wli
	 * 
	 *         Gets the Module string from the file name (e.g.
	 *         Accounts_0123.java = Accounts)
	 * 
	 * @param fileName
	 ...
	 * @author wli
	 * 
	 *         Check whether a string exist in the ModulesList array [helper
	 *         function]
	 * 
	 * @param mName
	 * @return a boolean value, true if the mName (module name) exist in the
	 *         ModuleList array, else return false
	 */
	private static boolean moduleExist(String mName) {
		for (String module : listOfModules) {
			if (module.equals(mName))
				return true;
		}
		return false;
	}
	 ...
	 *         place them into an array (ModulesList)
	 * 
	 * @param path
	 */
