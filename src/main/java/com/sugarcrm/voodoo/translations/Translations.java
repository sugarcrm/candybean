...
public class Translations {
	...
	// if the string to be replace is not in such module then search through all
	// DB modules
	...
	public static void main(String[] args) {
		try {
			if (args.length == 1)
				...
				Translate(args[0], args[1], args[2], args[3]);
			else {
				System.out
	 ...
	 * @author Wilson Li
	 * @param db
	 *            - database name (ie: translate_6_3, translate_6_2,
	 *            translate_6_1)
	 * @param mod
	 *            - module(s) name (ie: Account, Quotes, Contacts, etc)
	 * @param lang
	 *            - language (ie: fr_FR, ja_JP, zh_CH, en_UK, etc)
	 * @param testP
	 *            - path to a test file or to a folder containing multiple tests
	 * @throws Exception
	 */
	public static void Translate(String db, String mod, String lang,
			String testP) throws Exception {
		try {
			DATABASE = db;
			MODULE = mod;
			LANGUAGE = lang;
			TEST_PATH = testP;
			String defaultOutputPath = System.getProperty("user.dir")
					+ File.separator + "translated_Files_" + LANGUAGE;
			OUTPUT_FOLDER = getCascadingPropertyValue(null, defaultOutputPath,
					"translate.output");
	 ...
	 * @author Wilson Li
	 * @param propPath
	 *            - path to a properties file containing required translation
	 *            information (ie: translate.properties)
	 * @throws Exception
	 */
	public static void Translate(String propPath) throws Exception {
		try {
			...
			MODULE = getCascadingPropertyValue(translateProp, "",
					"translate.module");
			LANGUAGE = getCascadingPropertyValue(translateProp, "",
					"translate.language");
			TEST_PATH = getCascadingPropertyValue(translateProp, "",
					"translate.testpath");
	...
	/**
	 * A wrapper function that will recursively run all tests (translate) from
	 * the given path Condition: - If testPath is a test file then perform
	 * translation (fileReaderWriter) - If testPath is a folder then recursively
	 * run each item on the conditional check
	 * 
	 * @author Wilson Li
	 * @param testPath
	 *            - a path to a single test file or a folder containing multiple
	 *            test files
	 * @param outputFolder
	 *            - a path to a output folder
	 * @throws Exception
	 */
	private static void recursePathForTranslations(String testPath,
		...
		try {
			...
			if (inputFile.isFile()) { // testPath: Path to a test file
				...
				// If the inputFile is a file that contains a name from the
				// module(s) and is of java format then perform translation
				if (isModuleExist(moduleFileName)
						&& setAndCheckTestFormat(inputFile.getName())) {
					// perform translation
					String[] splitPathAndExtension = outputSubFolder
							.split("\\.");
				...
				for (File file : files) {
					...
					if (file.isFile() && isModuleExist(moduleFileName)
							&& setAndCheckTestFormat(file.getName())) {
						// perform translation
						String[] splitPathAndExtension = outputSubFolder
								.split("\\.");
						String splitPath = splitPathAndExtension[0];
						String splitExt = splitPathAndExtension[1];
						if (splitPathAndExtension.length == 1) {
							fileReaderWriter(moduleFileName, testPathSubFolder,
									splitPath + "_" + LANGUAGE);
						} else if (splitPathAndExtension.length == 2) {
							fileReaderWriter(moduleFileName, testPathSubFolder,
									splitPath + "_" + LANGUAGE + "." + splitExt);
						} else {
							printErrorMsg("Problem with path splitting, make sure there is at most 1 '.' in file name");
						}
					}
					// If the item is a directory, then recurse the function
					// with the item's path
					if (file.isDirectory()) {
	...
	/**
	 * Scan through the test file to replace (translate) the assert string to a
	 * language-specified string and write to a new file
	 * 
	 * @author Wilson Li
	 * @param module
	 *            - database module
	 * @param inputFile
	 *            - the test file to be translated
	 * @param outputFile
	 *            - translated test file
	 * @throws Exception
	 */
	private static void fileReaderWriter(String module, String inputFile,
		...
		Pattern pattern_assert = Pattern.compile(assertType); // set depending
																// on test file
																// format
		...
		Pattern pattern_moduletab = Pattern.compile("id=\"moduleTab_(" + module
				+ ")\"");
		Pattern pattern_menuextra_all = Pattern
				.compile("id=\"moduleTabExtraMenu(All)\"");
		...
		try {
			...
			while (fileScanner.hasNextLine()) {
				...
				if (match_assert.find()) { // Assert replacement
					if (TEST_FORMAT.equals("JAVA")) {
						String stringToBeReplaced = getToBeReplacedAssertString(match_assert
								.group(1));
						String ReplacementString = getDatabaseReplacementString(
								module, stringToBeReplaced);
						String newLine = line.replace(stringToBeReplaced,
								ReplacementString);
						output.write(newLine + "\r\n");
					} else {
						if (match_variable.find() || match_pageNumber.find()) {
							// printErrorMsg("Not finding translation for " +
							// match_assert.group(1));
							output.write(line + "\r\n");
						} else { // no strange characters, therefore proceed in
									// translation
							String newLine = line.replace(
									match_assert.group(1),
									getDatabaseReplacementString(module,
											match_assert.group(1)));
							output.write(newLine + "\r\n");
						}
					}
				...
				} else if (match_moduletab.find()) { // match_moduletab
														// replacement for (xml
														// only)
					String newLine = line.replace(match_moduletab.group(1),
							getDatabaseReplacementString("SugarFeed", "All")
									+ "_" + module);
					output.write(newLine);
				} else if (match_menuextra_all.find()) { // match_menuextra_all
															// replacement for
															// (xml only)
					String newLine = line.replace(
	...
	private static Connection connectToDatabase() throws Exception {
		...
		String serverName = getCascadingPropertyValue(translateProp, "",
				"translate.serverName");
		String username = getCascadingPropertyValue(translateProp, "",
				"translate.username");
		String password = getCascadingPropertyValue(translateProp, "",
				"translate.password");
		...
		try {
			...
			String url = "jdbc:mysql://" + serverName + File.separator
					+ DATABASE; // a JDBC url
			con = DriverManager.getConnection(url, username, password);
			printMsg("Connection to database successfull!");
			// PreparedStatement pst =
			// DB_CONNECTION.prepareStatement("SET NAMES utf8");
			// pst.executeQuery();
		} catch (Exception e) {
			throw new Exception(e.getMessage());
		} finally {
	...
	/**
	 * Queries the module table that is passed. However, if the translation does
	 * not exist within the current module, there will be a search in all
	 * modules by calling searchAllModules method (That is if SEARCH_ALL_MODULES
	 * is set to true)
	 * 
	 * @author Wilson Li
	 * @param module
	 ...
	 */
	@SuppressWarnings("finally")
	private static String getDatabaseReplacementString(String module,
			String englishString) throws Exception {
		String result = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		try {
			String query = "SELECT " + LANGUAGE + " FROM " + module
					+ " WHERE english = " + "\'" + englishString + "\'";
			pst = DB_CONNECTION.prepareStatement(query);
			rs = pst.executeQuery();

			// if there is a result due to the query, the translated value is
			// returned.
			if (rs.next()) {
			...
			} else if (SEARCH_ALL_MODULES) { // Search through the rest of the
												// modules
	...
	/**
	 * Perform a Query search on all modules. This can be called when a string
	 * is not found on a specified module. However it is currently disabled.
	 * Enable this feature by changing the constant SEARCH_ALL_MODULES to true.
	 * 
	 * @author Wilson Li
	 * @param englishString
	 * @return the translated string
	...
	private static String searchAllModules(String englishString)
		...
		while (tables[counter] != null) {
			...
			pst_ifExists = DB_CONNECTION.prepareStatement("SHOW columns from `"
					+ tables[counter] + "` where field='" + LANGUAGE + "'");
			...
			if (rs_ifExists.next()) {
				...
				pst_temp = DB_CONNECTION.prepareStatement("SELECT " + LANGUAGE
						+ " FROM " + tables[counter] + " WHERE english = "
						+ "\'" + englishString + "\'");
				...
				if (rs_temp.next()) {
					...
					// printErrorMsg("Could not find the translation for " +
					// englishString + " in the " + tables[counter] +
					// " module");
					if (tables[counter] == "WorkFlowTriggerShells") { // WorkFlowTriggerShells
																		// is
																		// the
																		// last
																		// module,
				...
				// printErrorMsg("The table: " + tables[counter] +
	 ...
	 * 
	 * @author Wilson Li
	 * @return an array of module names within the database that is being
	 *         queried
	 */
	...
	/**
	 * This function was implemented for Voodoo2 to replace the [Expected Value]
	 * from the following assert statement: Assert.assertEquals( [Message],
	 * [Expected Value], [Actual Value]); to a language-specific value retrieved
	 * from the database
	 * 
	 * @author Wilson Li
	 * @param assertStr
		...
		for (int index = 0; index < statement.length; index++) {
			if (statement[index].equals("\"") && !withinQuote) {
				...
				if (!prevString.equals("\\") && statement[index].equals("\"")) {
					// Is within quote but has reached the end fo the quoted
					// string
					prevString = statement[index];
	...
	/**
	 * This method will set the ASSERT_POSITION for specific string replacement
	 * in an assert statement. The position indicates the expectedValue of an
	 * assert statement. Two types of asserts currently support.
	 * 
	 * @author Wilson Li
	...
	private static void setAssertPosition(int numberOfArguments)
		...
		case 2:
			ASSERT_POSITION = 0;
			break; // assertEquals(expectedValue, actualValue);
		case 3:
			ASSERT_POSITION = 1;
			break; // assertEquals(message, expectedValue, actualValue);
		default:
			throw new Exception("Invalid Number of Arguments, got "
					+ numberOfArguments + " argument(s)");
		}
	}

	/**
	 * This will trim off any start and end quote marks from the string (if it
	 * has)
	 * 
	 * @author Wilson Li
	 * @param value
	 *            - a string that could contain a beginning and ending quote
	 *            marks
	 * @return - a string that does not contain any beginning and ending quote
	 *         marks
	 */
	...
	/**
	 * Get values from the properties file/System property with cascading
	 * functionality Precedence (left-to-right): System property > properties
	 * file > default value
	 * 
	 * @author Wilson Li
	 * @param props
	...
	private static String getCascadingPropertyValue(Properties props,
		...
		if (props != null) {
			...
			if (!props.containsKey(key) && value.equals("")) {
				printErrorMsg("System's property or property file does not contain such key: "
						+ key);
				System.exit(0);
			}
		} else {
	...
	/**
	 * Gets the Module string from the file name (e.g. Accounts_0123.java =
	 * Accounts)
	 * 
	 * @author Wilson Li
	 * @param fileName
	...
	/**
	 * Check whether a module name (String) exists in the listOfDatabaseModules
	 * list
	 * 
	 * @author Wilson Li
	 * @param moduleName
	 * @return a boolean value, true if the module name exists in the
	 *         listOfDatabaseModules list, else return false
	 */
	...
	/**
	 * Read from a file (a list of modules from the given path) and place them
	 * into a list (listOfDatabaseModules)
	 * 
	 * @author Wilson Li
	 * @param path
	 *            - can either be a path to a file containing a list of modules
	 *            or it can just be a single module name (ie, Accounts)
	 */
	...
	/**
	 * * This is to verify if the test file is either .java or .xml to * perform
	 * translations accordingly * * @param filename
	 * */
	private static boolean setAndCheckTestFormat(String filename) {
		if (filename.contains(".java")) {
			...
			printErrorMsg("file format is not either .java or .xml for the file: "
					+ filename);
			return false;
		}
		return true;
